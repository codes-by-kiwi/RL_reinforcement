# -*- coding: utf-8 -*-
"""simple_initial_model.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1hCmaBZVwLG1XcZZ9M_Dm2k1BXCCMPk2N
"""
...
cartpole_env = gym.make('CartPole-v1', new_step_api=True)
...
print(f"All available actions: {list(range(cartpole_env.action_space.n))}")
# there is 2 available actions since cart can move to the left or to the right
...
def bellman_update_training(current_state, action, immediate_reward, next_state, terminated, steps_taken=None, max_steps=None):
    ...
    q_values = model_sample.predict(state, verbose=0) # expected future rewards from current state
    ...
    model_sample.fit(state, q_values, verbose=0) #train model on updated q value

    q_values_updated = model_sample.predict(state, verbose=0)
...
print(k)
  ...
  #the step function in openai's gym environment is: https://stackoverflow.com/questions/73195438/openai-gyms-env-step-what-are-the-values

plt.plot(k)
plt.xlabel('Episode')
plt.ylabel('Total Reward')
plt.title('Q Learning with Neural Network Graph using CartPole')
plt.show()